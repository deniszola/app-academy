...
function greedyMakeChange(target, coins = [25, 10, 5, 1]) {
  ...
  function makeChange(target, coins) {
    ...
    if (target >= coins[0]) {
      return [coins[0], ...makeChange(target - coins[0], coins)];
    }

    return makeChange(target, coins.slice(1));
  }

  const change = makeChange(target, coins);
  const changeSum = change.reduce((sum, coin) => sum + coin);

  if (target - changeSum) return null;
  return change;
}
...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
  ...
  function makeChange(target, coins) {
    ...
    if (coins.length === 0 && target > 0) return [Infinity, []];
    if (coins[0] > target) return makeChange(target, coins.slice(1));

    const loseIt = makeChange(target, coins.slice(1));
    const useIt = makeChange(target - coins[0], coins);

    if (loseIt[0] < 1 + useIt[0]) return loseIt;
    else return [1 + useIt[0], useIt[1].concat(coins[0])];
  }

  const change = makeChange(target, coins);

  if (change[0] === Infinity) return null;
  return change[1];
}
