...
**classes/user.js** file. An instance of a `User` has a property of `name` and
a method of `changeName(newName)` that will set the `name` property to the
`newName` passed in as an argument.

Back in the **index.js** file, a function called
...
If you run the **index.js** file with `node index.js`, the following should be
...
so there will be an error thrown when running the **index.js** file.
